...
class Account(object):
    ...
    def categorize_new_bank_statement(Bank_statement_csv_name):
        ...
        #4) perform a loop for to categorize from the type to structure ==> New column "Groupes par structure"
        categorisation_df=Account.categorisation_definition()
        #get the nombre of unique type from
        list_unique=results_df.type.unique()
        
        #Initialisation of new column
        results_df["structure"]=""
        df=categorisation_df
        for i in list(results_df.type.unique()):
            ...
            results_df.loc[results_df.type.eq(i),"structure"]=list(structure_tmp)[0]   
        return results_df
